...
async def test_install_cf_client_pack_surfaces_failed_mod_downloads(
    tmp_path: Path, monkeypatch,
):
    """If any mod 403s (third-party-disabled), the install should raise
    with the offending mod identified — not silently boot half a pack."""
    pack_zip = _cf_client_zip(files=[
        {"required": True, "projectID": 100, "fileID": 7471280},
        {"required": True, "projectID": 200, "fileID": 7574260},
    ])
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if "forgecdn.net" in url:
            # The 7471280 / blocked one
            if "mod-7471280.jar" in url:
    ...
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    mp = Modpack(client=client)
    with pytest.raises(ModpackInstallError, match="failed to download"):
