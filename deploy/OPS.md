...
- **What it does:** serves the static landing (`/`) + server-list (`/play`)
  pages, `client/<sid>/*` artifacts, `servers.json`, and the client self-update
  binaries from R2 — all **static**, read-only, no box origin. The pages are
  generated by the box (`web/public_pages.py`) and published on `r2-publish`.
  The whole surface lives on **one host** (`play.<domain>`); apex + `www` 301 to it.
...
2. **Publish artifacts + pages** so R2 has current `index.html`/`play.html` +
   `client/<sid>/*` + `servers.json`:
   `curl -X POST localhost:8080/servers/<server_id>/r2-publish` (on the box).
   Set `NDRCHST_CLIENT_DOWNLOADS_BASE` (e.g. `https://play.<domain>/client`) so
   the pages link the standalone launcher binaries.
3. Verify: `play.<domain>/` (landing), `/play` (server list),
   `play.<domain>/client/<sid>/config.json`, `curl -I www.<domain>/` → 301 to play.
