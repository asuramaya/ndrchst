...
class Lifecycle:
    ...
    async def publish_to_r2(self, server_id: str) -> dict:
        """Push this server's client artifacts (incl. the small client.zip) +
        the static public pages to R2 so Cloudflare's edge serves them instead
        of the box. No-op (and says so) when R2 isn't configured. The big
        modpack is never pushed — the client pulls it from CurseForge's CDN."""
        import asyncio
        import os
